...
export class ShadersExtension implements WebviewExtension {
  ...
  constructor(
    ...
    preambleExtension: ShaderPreambleExtension,
    keyboardShaderExtension: KeyboardShaderExtension | undefined,
  ) {
    this.content = '';
      ...
      for (let texture of buffer.TextureInputs) {
        if (texture.Type === Types.TextureType.CubeMap) {
          preamble = preamble.replace(
            `sampler2D   iChannel${texture.Channel}`,
            `samplerCube iChannel${texture.Channel}`,
          );
        }
      }
      this.content += `\
