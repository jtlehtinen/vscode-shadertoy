...
export class ShaderParser {
  ...
  public next(): Object | ErrorObject | undefined {
    ...
    while (
      !this.lexer.eof() &&
      (nextToken === undefined || nextToken.type !== TokenType.PreprocessorKeyword)
    ) {
  ...
  private getTextureObject(
    previous: Token,
  ): Texture | TextureMagFilter | TextureMinFilter | TextureWrapMode | TextureType | ErrorObject {
    let nextToken = this.lexer.next();
    if (nextToken === undefined) {
      return this.makeError(
        `Expected string or "::" after "${previous.value}" but got end-of-file`,
      );
    }
    if (nextToken.type !== TokenType.String && nextToken.value !== '::') {
      return this.makeError(
        `Expected string or "::" after "${previous.value}" but got "${nextToken.value}"`,
      );
    }
  ...
  private getTextureParameter(
    index: number,
  ): TextureMagFilter | TextureMinFilter | TextureWrapMode | TextureType | ErrorObject {
    let nextToken = this.lexer.next();
    if (nextToken === undefined) {
      return this.makeError(
        `Expected texture parameter keyword after "::" but got end-of-file, valid options are "MinFilter", "MagFilter", "WrapMode" and "Type"`,
      );
    }
    if (nextToken.type !== TokenType.Keyword || nextToken.value === 'in') {
      return this.makeError(
        `Expected texture parameter keyword after "::" but got "${nextToken.value}", valid options are "MinFilter", "MagFilter", "WrapMode" and "Type"`,
      );
    }
    ...
    if (nextNextToken.type !== TokenType.String) {
      return this.makeError(
        `Expected string after "${textureSetting}" but got "${nextNextToken.value}"`,
      );
    }
    ...
    switch (textureSetting) {
      case 'MagFilter':
        if (
          Object.values(Types.TextureMagFilter).includes(settingValue as Types.TextureMagFilter)
        ) {
          ...
          return this.makeError(
            `Expected one of "Nearest" or "Linear" after "${textureSetting}" but got "${settingValue}"`,
          );
        }
      case 'MinFilter':
        if (
          Object.values(Types.TextureMinFilter).includes(settingValue as Types.TextureMinFilter)
        ) {
          ...
          return this.makeError(
            `Expected one of "Nearest", "NearestMipMapNearest", "NearestMipMapLinear", "Linear", "LinearMipMapNearest" or "LinearMipMapLinear" after "${textureSetting}" but got "${settingValue}"`,
          );
        }
      case 'WrapMode':
        if (Object.values(Types.TextureWrapMode).includes(settingValue as Types.TextureWrapMode)) {
          ...
          return this.makeError(
            `Expected one of "Clamp", "Repeat" or "Mirror" after "${textureSetting}" but got "${settingValue}"`,
          );
        }
      case 'Type':
        if (Object.values(Types.TextureType).includes(settingValue as Types.TextureType)) {
          ...
          return this.makeError(
            `Expected one of "Texture2D" or "CubeMap" after "${textureSetting}" but got "${settingValue}"`,
          );
        }
    }
  ...
  private getUniformObject(): Uniform | ErrorObject {
    ...
    if (nextToken !== undefined && nextToken.value === '=') {
      ...
      if (!this.testAssignable(type, defaultvalue.ValueType)) {
        return this.makeError(
          `Expected initializer of type assignable to "${type}" but got "${defaultvalue.LiteralString}" which is of type "${defaultvalue.ValueType}"`,
        );
      }
      nextToken = this.lexer.peek();
    }
    if (nextToken !== undefined && nextToken.value === 'in') {
      ...
      if (rangeArray.Value.length !== 2) {
        return this.makeError(
          `Expected array of type "${type}[2]" but got "${rangeArray.LiteralString}"`,
        );
      }
      ...
      if (!this.testAssignable(type, minValue.ValueType)) {
        return this.makeError(
          `Expected initializer of type "${type}" but got "${minValue.LiteralString}" which is of type ${minValue.ValueType}`,
        );
      }
      if (!this.testAssignable(type, maxValue.ValueType)) {
        return this.makeError(
          `Expected initializer of type "${type}" but got "${maxValue.LiteralString}" which is of type ${maxValue.ValueType}`,
        );
      }
      nextToken = this.lexer.peek();
    }
    if (nextToken !== undefined && nextToken.value === 'step') {
      ...
      if (!this.testAssignable(type, step.ValueType)) {
        return this.makeError(
          `Expected initializer of type "${type}" but got "${step.LiteralString}" which is of type ${step.ValueType}`,
        );
      }
    ...
    let isIntegerType =
      ['int', 'ivec2', 'ivec3', 'ivec4'].findIndex((value: string) => value === type) >= 0;
  ...
  private getValue(): LiteralNumber | LiteralValue | ErrorObject {
    ...
    if (nextToken.type === TokenType.Integer || nextToken.type === TokenType.Float) {
      ...
      if (!this.testAssignable(expectedType, firstValue.ValueType)) {
        return this.makeError(
          `Expected value assignable to type ${expectedType} but got ${firstValue.LiteralString} which is of type ${firstValue.Type}`,
        );
      }

      let values: LiteralNumber[] | LiteralValue[] =
        firstValue.Type === ObjectType.Number ? [firstValue] : [firstValue];
      ...
      for (let i = 1; i < expectedSize; i++) {
        ...
        if (!this.testAssignable(expectedType, nextValue.ValueType)) {
          return this.makeError(
            `Expected value assignable to type ${firstValue.ValueType} but got ${nextValue.LiteralString} which is of type ${nextValue.ValueType}`,
          );
        }
  ...
  private getArray(): LiteralValue | ErrorObject {
    ...
    let values: LiteralNumber[] | LiteralValue[] =
      currentValue.Type === ObjectType.Number ? [currentValue] : [currentValue];

    while (true) {
      nextToken = this.lexer.next();
      if (nextToken === undefined) {
        return this.makeError(
          `Expected "," or "}" after "${currentValue.LiteralString}" but got end-of-file`,
        );
      }
      if (nextToken.value !== ',' && nextToken.value !== closePunc) {
        return this.makeError(
          `Expected "," or "}" after "${currentValue.LiteralString}" but ${nextToken.value}`,
        );
      }
      ...
      if (!this.testAssignable(type, currentValue.ValueType)) {
        return this.makeError(
          `Expected value assignable to type ${type} but got "${currentValue.LiteralString}" which is of type ${currentValue.ValueType}`,
        );
      }
  ...
  private testAssignable(leftType: string, rightType: string) {
    ...
    if (leftIsArray || rightIsArray) {
      let arrayElementTypesAssignable = this.testAssignable(
        leftTypeDesc.BaseTypeName,
        rightTypeDesc.BaseTypeName,
      );
    ...
    if (
      (leftIsVecType as boolean) &&
      !rightIsArray &&
      this.testAssignable(leftTypeDesc.BaseTypeName, rightTypeDesc.BaseTypeName)
    ) {
      return true;
    }
    ...
    if (
      leftUndefinedArraySize >= 0 &&
      leftUndefinedArraySize < leftTypeDesc.ArraySizes.length - 1
    ) {
      return false;
    }
    ...
    if (
      rightUndefinedArraySize >= 0 &&
      rightUndefinedArraySize < rightTypeDesc.ArraySizes.length - 1
    ) {
      return false;
    }

    let arraySizesMatch = leftTypeDesc.ArraySizes.every(
      (arraySize: number, index: number) =>
        arraySize === 0 || arraySize === rightTypeDesc.ArraySizes[0],
    );
