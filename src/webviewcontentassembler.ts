...
export class WebviewContentAssembler {
  ...
  private insertModule(webviewModule: WebviewModule) {
    let sortedIndex = this.webviewModules.findIndex(
      (module: WebviewModule) => module.LineNumber < webviewModule.LineNumber,
    );
    if (sortedIndex < 0) {
  ...
  public assembleWebviewContent() {
    for (let webviewModule of this.webviewModules) {
      if (webviewModule.Module.Type === ModuleType.Insert) {
        this.webviewContent.insertAfterLine(
          webviewModule.Module.Extension.generateContent(),
          webviewModule.LineNumber,
        );
      } else {
        this.webviewContent.replaceWithinLine(
          webviewModule.Module.ReplaceContent,
          webviewModule.Module.Extension.generateContent(),
          webviewModule.LineNumber,
        );
      }
