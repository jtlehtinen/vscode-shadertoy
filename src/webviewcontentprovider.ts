...
export class WebviewContentProvider {
  ...
  public parseShaderTree(generateStandalone: boolean): string[] {
    ...
    // Parse Shaders
    {
      ...
      new BufferProvider(this.context).parseShaderCode(
        shaderName,
        shader,
        this.buffers,
        this.commonIncludes,
  ...
  public generateWebviewContent(
    webview: vscode.Webview | undefined,
    startingState: Types.RenderStartingData,
  ): string {
    let generateStandalone = webview === undefined;

    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // Resource Helpers
    let makeWebviewResource =
      webview !== undefined
        ? (localPath: string) =>
            this.context.makeWebviewResource(webview, this.context.makeUri(localPath)).toString()
        : (localPath: string) => localPath;
    let getWebviewResourcePath =
      webview !== undefined
    ...
    this.webviewAssembler.addReplaceModule(
      ...
      '<!-- Preamble Line Numbers -->',
    );

    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // Custom Uniforms
    if (useUniforms) {
      let uniformsInitExtension = new UniformsInitExtension(
        this.buffers,
        startingState.UniformsGui,
      );
    ...
    // Fix up line offsets
    {
      ...
      for (let buffer of this.buffers) {
        buffer.LineOffset +=
          preambleExtension.getShaderPreambleLineNumbers() + webglPlusThreeJsLineNumbers;
    ...
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // Shader Scripts
    let shadersExtension = new ShadersExtension(
      this.buffers,
      preambleExtension,
      keyboardShaderExtension,
    );
    ...
    // Audio Logic
    if (useAudio) {
      let audioInitExtension = new AudioInitExtension(
        this.buffers,
        this.context,
        makeWebviewResource,
      );
    ...
    if (!generateStandalone) {
      if (this.context.getConfig<boolean>('showPauseButton')) {
        ...
        this.webviewAssembler.addWebviewModule(
          pauseButtonStyleExtension,
          '/* Pause Button Style */',
        );
    ...
    if (!generateStandalone) {
      if (this.context.getConfig<boolean>('showScreenshotButton')) {
        let screenshotButtonStyleExtension = new ScreenshotButtonStyleExtension(
          getWebviewResourcePath,
        );
        this.webviewAssembler.addWebviewModule(
          screenshotButtonStyleExtension,
          '/* Screenshot Button Style */',
        );

        let screenshotButtonExtension = new ScreenshotButtonExtension();
        this.webviewAssembler.addWebviewModule(
          screenshotButtonExtension,
          '<!-- Screenshot Element -->',
        );
      }
    }
    let forcedScreenshotResolution =
      this.context.getConfig<[number, number]>('screenshotResolution');
    if (forcedScreenshotResolution === undefined) {
      forcedScreenshotResolution = [-1, -1];
    }
    let forcedScreenshotResolutionExtension = new ForcedScreenshotResolutionExtension(
      forcedScreenshotResolution,
    );
    this.webviewAssembler.addReplaceModule(
      forcedScreenshotResolutionExtension,
      'let forcedScreenshotResolution = [<!-- Forced Screenshot Resolution -->];',
      '<!-- Forced Screenshot Resolution -->',
    );
