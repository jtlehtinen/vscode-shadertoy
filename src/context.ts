...
export class Context {
  ...
  public getResourceUri(file: string): vscode.Uri {
    return this.makeUri(path.join(this.context.extensionPath, 'resources', file));
  }
  ...
  public mapUserPath(userPath: string, sourcePath: string): { file: string; userPath: string } {
    ...
    let file = ((file: string) => {
      ...
      if (vscode.workspace.workspaceFolders !== undefined) {
        ...
        if (workspaceFileCandidates.length > 1) {
          vscode.window.showErrorMessage(
            `Multiple candidates for file '${userPath}' were found in your workspace folders, first option was picked: '${workspaceFileCandidates[0]}'`,
          );
        }
      ...
      vscode.window.showErrorMessage(
        `File '${userPath}' was not found, paths that were tried were\n\t${fileCandidates.join(
          '\n\t',
        )}`,
      );
      return file;
    ...
    newDocument.then(
      (document: vscode.TextDocument) => {
        ...
        for (let diagnostic of diagnosticBatch.diagnostics) {
          ...
          this.collectedDiagnostics[file].push(
            new vscode.Diagnostic(range, diagnostic.message, severity),
          );
        }
  ...
  public revealLine(file: string, line: number) {
    let highlightLine = (document: vscode.TextDocument, line: number) => {
      ...
      let range = document.lineAt(line).range;
      vscode.window
        .showTextDocument(document, vscode.ViewColumn.One, true)
