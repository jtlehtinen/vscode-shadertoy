...
export class BufferProvider {
  ...
  public parseShaderCode(
    file: string,
    code: string,
    buffers: Types.BufferDefinition[],
    commonIncludes: Types.IncludeDefinition[],
  ...
  private parseShaderCodeInternal(
    rootFile: string,
    file: string,
    code: string,
    buffers: Types.BufferDefinition[],
      ...
      switch (mimeType) {
        case 'text': {
          if (depFile === 'self' || depFile === file) {
            ...
            // Parse the shader
            this.parseShaderCodeInternal(
              rootFile,
              depFile,
              shaderFile.bufferCode,
              buffers,
              commonIncludes,
              generateStandalone,
            );
        ...
        case 'audio': {
          if (this.context.getConfig<boolean>('enabledAudioInput')) {
            ...
            vscode.window.showWarningMessage(
              'You are trying to use an audio file, which is currently disabled in the settings.',
            );
          }
          break;
        }
...

    {
      ...
      if (versionPos === 0) {
        ...
        this.showInformationAtLine(
          file,
          `Version directive '${versionDirective}' ignored by shader-toy extension`,
          0,
        );
      }
    }
...
void main() {
    ...
    mainImage(gl_FragColor, fragCoord);
      ...
      if (this.context.getConfig<boolean>('shaderToyStrictCompatibility') || strictComp.Value) {
        ...
        let mainImagePos = code.search(
          /void\s+mainImage\s*\(\s*out\s+vec4\s+\w+,\s*(in\s)?\s*vec2\s+\w+\s*\)\s*\{/g,
        );
        ...
        for (let i = 0; i < 9; i++) {
          if (code.search(`iChannel${i}`) > 0) {
            if (definedTextures[i] === undefined) {
              vscode.window
                .showWarningMessage(
                  `iChannel${i} in use but there is no definition #iChannel${i} in shader`,
                  'Details',
                )
                .then(() => {
                  vscode.window.showInformationMessage(
                    `To use this channel add to your shader a line '#iChannel${i}' followed by a space and the path to your texture. Use 'file://' for local textures, 'https://' for remote textures or 'buf://' for other shaders.`,
                  );
                });
            }
          }
    ...
    while (!parser.eof()) {
      ...
      switch (nextObject.Type) {
        ...
        case ObjectType.Texture: {
          ...
          if (local) {
            ...
            if (userPath === 'self') {
              ...
              ({ file: textureFile, userPath: userPath } = this.context.mapUserPath(
                userPath,
                file,
              ));
        ...
        case ObjectType.Include: {
          ...
          if (sharedIncludeIndex < 0) {
            ...
            if (includeCode.success) {
              ...
              let transformedIncludeCode = this.transformCode(
                rootFile,
                includeFile,
              ...
              this.showErrorAtLine(
                file,
                `Failed opening include file "${includeFile}"`,
                parser.line(),
              );
            }
          }
        ...
        case ObjectType.Uniform: {
          if (
            nextObject.Default !== undefined &&
            nextObject.Min !== undefined &&
            nextObject.Max !== undefined
          ) {
          ...
          if (nextObject.Default === undefined && nextObject.Min !== undefined) {
            ...
            this.showDiagnosticAtLine(
              file,
              'Custom uniform specifies no default value, the minimum of its range will be used.',
              parser.line(),
              vscode.DiagnosticSeverity.Information,
            );
          }
          ...
          if (nextObject.Default === undefined) {
            this.showErrorAtLine(
              file,
              'Can not deduce default value for custom uniform, either define a default value or range',
              parser.line(),
            );
          } else {
